    ...
    public class Stardust : RUBot
    {
        ...
        public override void Run()
        {
            ...
            if (ShouldAttack() && IsClosest(Me, true) && Action == null)
            {
                ...
                Action = shot != null ? shot : GetOffensiveAction();
            }

            if (ShouldAttack() && IsSecondClosest() && GetClosestTeammate().IsGrounded && Action == null)
            {
                Shot shot = FindShot(DefaultShotCheck, new Target(TheirGoal));
                Action = shot != null ? shot : GetSupportingAction();
            }

            if (ShouldDefend() && IsClosest(Me, true) && Action == null)
            {
                Shot shot = FindShot(DefaultShotCheck, new Target(TheirGoal));
                Action = shot != null ? shot : GetDefensiveAction();
            }

            if (ShouldDefend() && IsSecondClosest() && GetClosestTeammate().IsGrounded && Action == null)
            {
                Shot shot = FindShot(DefaultShotCheck, new Target(TheirGoal, true));
                Action = shot != null ? shot : GetSupportingDefenseAction();
            }
        }
        ...
        private IAction GetOffensiveAction()
        {
            ...
            if (GetClosestOpponent().Location.Dist(Ball.Location) > GetClosestTeammate().Location.Dist(Ball.Location))
            {
                return new GetBoost(Me);
            }
