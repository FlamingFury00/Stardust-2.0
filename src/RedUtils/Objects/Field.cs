...
namespace RedUtils
{
    /// <summary>Basic field geometry and drivable surfaces.</summary>
    public static class Field
    {
        ...
        public static void Update(GamePacketT packet)
        {
            ...
            for (int i = 0; i < System.Math.Min(packet.BoostPads.Count, Boosts.Count); i++) Boosts[i].Update(packet.BoostPads[i]);
        }
        ...
        public static Surface NearestSurface(Vec3 pos) => Nearest(pos, DrivableSurfaces, null);
        public static Surface NearestSurface(Vec3 pos, Surface[] excludedSurfaces) => Nearest(pos, Surfaces, excludedSurfaces);

        /// <summary>Equivalent ordered minimum without copying the surface collection for every ETA query.</summary>
        private static Surface Nearest(Vec3 pos, Dictionary<string, Surface> candidates, Surface[] excluded)
        {
            Surface closest = null;
            float closestDistance = float.PositiveInfinity;
            foreach (Surface surface in candidates.Values)
            {
                if (excluded != null && Array.IndexOf(excluded, surface) >= 0) continue;
                float distance = pos.Dist(surface.Limit(pos));
                if (closest == null || distance < closestDistance)
                { closest = surface; closestDistance = distance; }
            }
            return closest ?? throw new InvalidOperationException("No eligible surfaces.");
        }
