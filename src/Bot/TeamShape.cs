...
namespace Bot
{
    ...
    /// <summary>One election from the same world snapshot; never seed a team election with the observing bot.</summary>
    public static class TeamShape
    {
        public static TeamAssignment Assign(IEnumerable<Car> cars, int team, Vec3 ball, Func<Car, float> eta)
        {
            ...
            foreach (Car car in living)
            {
                ...
                // A returning, ball-side car is not automatically the best challenger.
                ...
                if (result.LastBack < 0 || Deeper(car, living.Find(c => c.Index == result.LastBack), side))
                    result.LastBack = car.Index;
            }
            foreach (Car car in living)
                if (car.Index != result.FirstMan && (result.Cover < 0 ||
                    Deeper(car, living.Find(c => c.Index == result.Cover), side))) result.Cover = car.Index;
            if (result.Cover < 0) result.Cover = result.FirstMan;
            int supportIndex = 0;
            foreach (Car car in living)
            {
                TeamRole role = car.Index == result.FirstMan ? TeamRole.Challenger :
                    car.Index == result.Cover ? TeamRole.Cover : TeamRole.Support;
                result.Roles[car.Index] = role;
                result.Targets[car.Index] = Target(ball, Field.Goals[team].Location, role,
                    role == TeamRole.Support ? supportIndex++ : 0);
            }
            return result;
        }

        private static bool Deeper(Car a, Car b, int side) => b == null ||
            a.Location.y * side > b.Location.y * side + 1 ||
        ...
        public static Vec3 Target(Vec3 ball, Vec3 goal, TeamRole role, int slot = 0)
        {
            ...
            // A fixed team-relative tie break also mirrors correctly when the ball is centered.
            float wing = MathF.Abs(ball.x) > 200 ? -MathF.Sign(ball.x) : -side;
            if (role == TeamRole.Cover)
                return new Vec3(System.Math.Clamp(ball.x * 0.18f, -550, 550),
                    side * System.Math.Clamp(depth + 2450, -800, 4400), 17);
            if (role == TeamRole.Support)
                return new Vec3(wing * (slot % 2 == 0 ? 1750 : -1750),
                    side * System.Math.Clamp(depth + 1200 + slot / 2 * 600, -3400, 3150), 17);
            return new Vec3(System.Math.Clamp(ball.x * 0.75f, -3100, 3100),
                side * System.Math.Clamp(depth + 850, -3900, 4350), 17);
        }
        ...
        public static bool HasCover(IEnumerable<Car> cars, Car me, Vec3 ball, int team)
        {
            ...
            foreach (Car car in cars)
                if (car.Index != me.Index && car.Team == team && !car.IsDemolished && car.Location.z < 350 &&
                    car.Location.y * side >= ball.y * side + 250 && MathF.Abs(car.Location.x) < 2600)
                    return true;
            return false;
        }
    }
